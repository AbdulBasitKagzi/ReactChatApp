...
function Conference() {
  ...
  const namePeers = peers.map(({ name }) => {
    return name;
  });
  console.log("name", namePeers);

  const demoname = demoPeer.map((demo) => {
    return demo.name;
  });
  console.log("demonname", demoname);

  let identifier;
  if (demoname.includes(namePeers[0])) {
    identifier = demoPeer.filter((demo, index) => {
      console.log("alex", demo.name);
      if (namePeers[0] === demo.name) {
        return demo.id;
      }
    });
  ...
  const newPeers = peers.map(({ name, id }, index) => {
    return { name, id, ...identifier[index] };
  });
  ...
  //     localPeerStats.subscribe?.bytesReceived
  //   );
  ...
  //   console.log("peerStats", useHMSStatsStore(selectHMSStats.localPeerStats));
  //   React.useEffect(() => {
  //     console.log("peerStats", localPeerStats);
  ...
  return (
    <div className="conference-section">
      <h2>Conference</h2>
      {/* <Timer peersLength={peers.length} /> */}
      ...
      <button
        onClick={async () => {
          ...
          // console.log(await hmsActions.endRoom);
